...
output "validation_data" {
  ...
  value = {
    ...
    dns_resolution = {
      cluster_endpoint_hostname = length(split("//", google_container_cluster.primary.endpoint)) > 1 ? split("//", google_container_cluster.primary.endpoint)[1] : google_container_cluster.primary.endpoint
      private_cluster          = var.enable_private_cluster
    }
