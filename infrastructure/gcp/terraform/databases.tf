...
# Using a more specific metric filter for memory utilization
resource "google_monitoring_alert_policy" "redis_memory_utilization" {
  ...
  conditions {
    ...
    condition_threshold {
      filter          = "resource.type=\"redis_instance\" AND metric.type=\"redis.googleapis.com/stats/memory/usage_ratio\" AND resource.label.instance_id=\"${google_redis_instance.main_cache[0].name}\""
